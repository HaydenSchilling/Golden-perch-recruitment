...
mydata_wide <- mydata_wide %>% mutate(pre3 = Date %m-% months(0),
...
flow_dat4 <- read_csv("Darling at Wilcannia Main Channel 425008 Flow Temp.csv") %>%
  ...
  mutate(Lg_Achieved = case_when(Lg_Duration == 15 ~ 1,
         ...
         Overbank_Achieved = case_when(OverBank_Duration == 15 ~ 1,
                                        T ~ 0)) #%>% filter(Date>as.Date("1990-01-01"))
# 
# ggplot(flow_dat4, aes(Date, Flow, col = as.factor(Overbank_Achieved))) + geom_line()
...
for(i in 1:nrow(mydata_wide)){
  if(mydata_wide$WaterbodyName2[i] == "Warrego River"){
    ...
    mydata_wide$Over_bank[i] <- max(temp_flow$OverBank_Duration, na.rm=T)
...
write_csv(mydata_wide2, "GP YOY modelling data_full_year_test.csv")
mydata_wide2 <- read_csv("GP YOY modelling data_full_year_test.csv")
...
MuMIn::AICc(f1,f2,f3,f4,f5,f6) # full model clearly the best
...
ggplot(predict_dat_final, aes(Mean_Flow_unscaled, exp(Predicted_GP))) + geom_line() +
  ...
  theme(axis.text = element_text(colour="black", size=12),
        ...
        strip.background = element_blank(),
        panel.border = element_rect(colour="black", fill=NA))+
  ylab("Predicted GP YOY ± SE (90s e-Fishing)") + xlab("Mean daily flow 3-6 month prior to sampling (ML/Day)")
...
ewr_Sample_year <- table(mydata_wide3$EWR, mydata_wide3$WaterbodyName2)

write_csv(as.data.frame(ewr_Sample_year), "EWR Sample size full year.csv")
...
AIC(e1,e2,e3,e4,e5) # e2 is clear winner

e6 <- glmmTMB::glmmTMB(GP_YOY ~ WaterbodyName2 + # drop EWR
                         scale(stocking)+
                         #(1|SamplingRecordID) , offset = log(Sampling_duration),
                         (1|SiteID) + (1|SampleDate), offset = log(Sampling_duration),
                       data = as.data.frame(mydata_wide3), family = nbinom1())

e7 <- glmmTMB::glmmTMB(GP_YOY ~ EWR+ + # drop Waterbody
                         scale(stocking)+
                         #(1|SamplingRecordID) , offset = log(Sampling_duration),
                         (1|SiteID) + (1|SampleDate), offset = log(Sampling_duration),
                       data = as.data.frame(mydata_wide3), family = nbinom1())

e8 <- glmmTMB::glmmTMB(GP_YOY ~ EWR+WaterbodyName2 + # drop stocking
                         #scale(stocking)+
                         #(1|SamplingRecordID) , offset = log(Sampling_duration),
                         (1|SiteID) + (1|SampleDate), offset = log(Sampling_duration),
                       data = as.data.frame(mydata_wide3), family = nbinom1())

AIC(e2,e6,e7,e8) # e2 is clear winner

summary(e2)
car::Anova(e2)
eee <- as.data.frame(car::Anova(e2))
eee$Parameter <- row.names(eee)
#write_csv(eee, "EWR model results.csv")

performance::r2_nakagawa(e2, tolerance = 1e-10)
r2(e2)
...
library(DHARMa)
resids <- simulateResiduals(e2)
plot(resids)
testOutliers(e1, type="bootstrap")
testDispersion(e1)

plot(effects::allEffects(e2))

plot(ggeffects::ggpredict(e2, terms=c("EWR", "WaterbodyName2")))
...
pred_dat <- predict(e2, newdata = new_data, se=T, type="response")
...
ggplot(new_data, aes(EWR2, PredictedFit)) + geom_point()+# facet_wrap(~WaterbodyName2)+
  #scale_y_log10()+
  theme_classic()+
  geom_errorbar(aes(ymin=lower_CI, ymax=upper_CI), alpha=0.5) +
  scale_x_discrete(breaks=c("None", "Large\nFresh"), limits = c("None", "Large\nFresh"))+
  ylab("Predicted GP YOY ± SE\n(90s e-Fishing)")+
  xlab("EWR")+
  theme(axis.text = element_text(colour="black", size=12),
        axis.title = element_text(face="bold", size=14),
...
ggsave("EWR Achievement with duration_full_year.png", width = 21/2, height=14.8/2, units="cm", dpi=300)
