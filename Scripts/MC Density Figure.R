...
key_rivers <- c("Lachlan River", "Macquarie River", "Gwydir River", "Namoi River", "Darling River",
                ...
                "Severn River", "Dumaresq River", "Macintyre River")
...
ggsave("Murray cod YOY exploration.png", dpi=600, width =21, height=14.8, units="cm")
...
ggplot(gp_bio_expanded2, aes(x=Length_mm,y=FinYear, group=FinYear, fill = Mean_Length)) + 
  geom_density_ridges(rel_min_height=0.05) + facet_wrap(~WaterbodyName) + xlim(c(0,650))+
  scale_fill_viridis_c(option = "turbo", name="Mean\nLength (mm)") + theme_bw() + ylab("Sampling Year Ending June 30th") + xlab("Length (mm)") +
...
ggsave("MC Juvenile Densities.png", dpi=600, width = 21, height=18, units="cm")
ggsave("MC Juvenile Densities.pdf", dpi=600, width = 21, height=18, units="cm")
...
ggplot(gp_bio_expanded3, aes(x=Length_mm,y=FinYear, group=FinYear, fill = Mean_Length)) + #
  stat_density_ridges(quantile_lines = T, quantiles = 2, rel_min_height=0.05, from=0)+
  xlim(c(0, 1250))+
...
ggsave("MC Length density plot coloured2.png", dpi=600, units="cm",
       width=21, height=21)
